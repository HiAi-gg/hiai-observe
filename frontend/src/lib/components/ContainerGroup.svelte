...
import { highlightJson, isJson, isStackTrace, stripAnsi } from "$lib/utils";
...
function levelBg(level: string) {
  if (level === "error") return "bg-[var(--destructive)]/15";
  if (level === "warn") return "bg-[var(--warning)]/15";
  return "";
}
</script>
  ...
  {#each groups as g (g.name)}
    ...
    <div class="overflow-hidden rounded-lg border border-[var(--border)]">
      <button type="button"
        ...
        class="flex w-full items-center gap-3 bg-[var(--card)] px-3 py-2 text-left text-xs transition-colors hover:bg-[var(--accent)]"
      >
        <svg class="h-3 w-3 shrink-0 text-[var(--muted-foreground)] transition-transform {isExpanded ? 'rotate-90' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
        <span class="font-medium text-[var(--muted-foreground)]">{g.name}</span>
        <span class="text-[var(--muted-foreground)]">{g.entries.length.toLocaleString()}</span>
        {#if g.errorCount > 0}
          <span class="rounded-full bg-[var(--destructive)]/15 px-1.5 text-[10px] font-semibold text-[var(--destructive)]">{g.errorCount}</span>
        {/if}
        {#if g.warnCount > 0}
          <span class="rounded-full bg-[var(--warning)]/15 px-1.5 text-[10px] font-semibold text-[var(--warning)]">{g.warnCount}</span>
        {/if}
      </button>
          ...
          <table class="w-full font-mono text-xs">
            <tbody>
              {#each g.entries as log (log.id)}
                ...
                <tr class="border-t border-[var(--border)] {levelBg(log.level)} align-top">
                  ...
                  <td class="px-3 py-1 text-[var(--foreground)] break-all">
                    ...
                    {:else if isStack}
                      ...
                      {#if expandedStack.has(log.id)}
                        <pre class="mt-1 overflow-x-auto rounded bg-[var(--card)] p-2 text-xs">{#each lines as line, i}<span class="{line.trimStart().startsWith('at ') ? 'text-[var(--destructive)]' : 'text-[var(--muted-foreground)]'}">{line}</span>{#if i < lines.length - 1}{"\n"}{/if}{/each}</pre>
                      {:else}
