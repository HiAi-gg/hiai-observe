...
function itemClass(active: boolean): string {
  return active
    ? `${base} bg-[var(--primary)]/10 text-[var(--primary)]`
    : `${base} text-[var(--muted-foreground)] hover:bg-[var(--background)]`;
}
</script>

<aside class="flex w-56 shrink-0 flex-col gap-1 overflow-y-auto rounded-lg border border-[var(--border)] bg-[var(--card)] p-3 text-xs">
  <div class="mb-2 flex items-center justify-between">
    <span class="font-medium text-[var(--muted-foreground)]">Containers</span>
    <span class="text-[var(--muted-foreground)]">{containers.length}</span>
  </div>
  ...
  {#each containers as c (c.name)}
    ...
    <div class="mx-2 h-1 overflow-hidden rounded-full bg-[var(--accent)]">
      <div
        class="h-full rounded-full bg-[var(--primary)]/40"
