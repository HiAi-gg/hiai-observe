...
<div class="flex items-center gap-1">
  {#each presets as p (p.key)}
    <button type="button"
      ...
      class:bg-[var(--primary)]={value === p.key}
      class:text-[var(--primary-foreground)]={value === p.key}
      class:border={value !== p.key}
      class:border-[var(--border)]={value !== p.key}
      class:text-[var(--muted-foreground)]={value !== p.key}
      class:hover:bg-[var(--card)]={value !== p.key}
    >
      {p.label}
    </button>
  {/each}
  <button type="button"
    ...
    class:border-[var(--primary)]={value === "custom" || showCustom}
    class:text-[var(--primary)]={value === "custom" || showCustom}
    class:border-[var(--border)]={value !== "custom" && !showCustom}
    class:text-[var(--muted-foreground)]={value !== "custom" && !showCustom}
...
{#if showCustom}
  <div class="flex items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-xs">
    <label class="flex items-center gap-1 text-[var(--muted-foreground)]">
      From
      <input
        type="datetime-local"
        value={toDatetimeLocal(customFrom)}
        onchange={(e) => { customFrom = (e.target as HTMLInputElement).value; }}
        class="rounded border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-[var(--foreground)] focus:border-[var(--primary)] focus:outline-none"
      />
    </label>
    <label class="flex items-center gap-1 text-[var(--muted-foreground)]">
      To
    ...
    <button type="button"
      onclick={applyCustom}
      class="rounded-md bg-[var(--primary)] px-3 py-1 text-xs text-[var(--primary-foreground)] hover:opacity-90"
    >
      Apply
    </button>
  </div>
