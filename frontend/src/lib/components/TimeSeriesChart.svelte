...
<div class="relative">
  {#if label}
    <h3 class="mb-2 text-sm font-medium text-[var(--muted-foreground)]">{label}</h3>
  {/if}

  {#if data.length === 0}
    <div class="flex items-center justify-center text-sm text-[var(--muted-foreground)]" style="height: {height}px">
      No data available
    </div>
  {:else}
    <svg
    ...
    >
      ...
      {#each yTicks() as tick}
        ...
        <line
          ...
          stroke="var(--border)" stroke-dasharray="4,4" stroke-width="0.5"
        />
        <text x={padding.left - 8} y={y + 4} text-anchor="end" class="fill-[var(--muted-foreground)] text-xs">
          {tick.toFixed(0)}{unit}
        </text>
      {/each}
      ...
      {#each xTicks() as tick}
        ...
        <text x={x} y={height - 8} text-anchor="middle" class="fill-[var(--muted-foreground)] text-xs">
      ...
      {#if tooltip}
        <line
          ...
          stroke="var(--muted-foreground)" stroke-dasharray="2,2" stroke-width="0.5"
        />
        <circle cx={tooltip.x} cy={tooltip.y} r="5" fill={color} stroke="var(--card)" stroke-width="2" />
        <rect
          x={tooltip.x - 50} y={tooltip.y - 30}
          width="100" height="22" rx="4"
          fill="var(--card)" stroke="var(--border)" stroke-width="0.5"
        />
        <text x={tooltip.x} y={tooltip.y - 15} text-anchor="middle" class="fill-[var(--foreground)] text-xs font-medium">
