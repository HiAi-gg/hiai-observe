...
const navItems = [
  ...
  },
  {
    ...
    children: [
      { href: resolve("/traces/agents"), label: "Agents" },
      { href: resolve("/traces/models"), label: "Models" },
      { href: resolve("/traces/workflows"), label: "Workflows" },
    ],
  },
  {
    ...
    children: [
      { href: resolve("/settings/notifications"), label: "Notifications" },
      { href: resolve("/settings/alert-history"), label: "Alert History" },
      { href: resolve("/settings/retention"), label: "Retention" },
      { href: resolve("/settings/team"), label: "Team" },
    ],
  },
];
  ...
  <div class="flex h-screen overflow-hidden bg-[var(--background)]">
        ...
        {#if projectDropdownOpen}
          <div class="absolute left-3 right-3 z-50 mt-1 rounded-md border border-[var(--border)] bg-[var(--card)] py-1 shadow-lg">
            ...
            <div class="border-t border-[var(--border)]">
              <a
                href="{resolve('/settings')}#projects"
    ...
    <nav class="flex-1 space-y-1 p-2">
      {#each navItems as item (item.href)}
        {@const active = item.href === resolve("/") ? (page.url.pathname === item.href || page.url.pathname === `${item.href}/`) : page.url.pathname.startsWith(item.href)}
