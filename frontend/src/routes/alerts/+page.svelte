...
<div class="p-6 max-w-[1400px] mx-auto space-y-6">
  ...
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
      <h1 class="text-2xl font-bold text-[var(--foreground)]">Alert Channels</h1>
  ...
  {#if error}
    <div class="flex items-center gap-3 rounded-lg border border-[var(--destructive)]/50 bg-[color-mix(in_oklch,var(--destructive)_18%,transparent)] px-4 py-3 text-sm text-[var(--destructive)]">
      ...
      <button type="button" onclick={() => loadData()} class="rounded border border-[var(--destructive)]/50 px-2.5 py-1 text-xs text-[var(--destructive)] hover:bg-[color-mix(in_oklch,var(--destructive)_18%,transparent)] transition-colors">Retry</button>
    </div>
  {/if}
  ...
  {#if loading}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {#each Array(10) as _, i (i)}
        <div class="h-40 animate-pulse rounded-xl bg-[var(--card)]" style="opacity: {1 - i * 0.08}"></div>
      {/each}
    </div>
  {:else}
    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {#each channels as channel (channel.type)}
        <div class="flex flex-col justify-between rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 hover:border-[var(--primary)]/30 hover:shadow-md transition-all duration-200">
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-bold text-[var(--foreground)]">{channel.name}</h3>
            ...
            <p class="text-sm text-[var(--muted-foreground)] leading-relaxed min-h-[40px]">
              {channel.description}
            </p>
          </div>

          <div class="mt-5 pt-4 border-t border-[var(--border)]/50 flex items-center justify-between">
            <span class="text-xs text-[var(--muted-foreground)]">
              {channel.configFields.length} config fields
            </span>
            <button type="button"
              onclick={() => openConfigure(channel)}
              class="rounded-lg bg-[var(--primary)]/10 px-3.5 py-1.5 text-xs font-bold text-[var(--primary)] hover:bg-[var(--primary)]/20 transition-all duration-150"
            >
              Configure
            </button>
          </div>
  ...
  {#if modalOpen && activeChannel}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs" role="dialog" aria-modal="true">
      <div class="w-full max-w-lg rounded-xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
        <!-- Modal Header -->
        <div class="flex items-center justify-between border-b border-[var(--border)] pb-3">
          <div>
            ...
            <p class="text-xs text-[var(--muted-foreground)] mt-0.5">
              Source: <span class="font-semibold capitalize text-[var(--muted-foreground)]">{configSource}</span>
            </p>
          </div>
        ...
        {#if loadingConfig}
          <div class="py-12 flex flex-col items-center justify-center gap-3">
            <div class="h-6 w-6 animate-spin rounded-full border-2 border-[var(--primary)] border-t-transparent"></div>
            <span class="text-xs text-[var(--muted-foreground)]">Loading configuration...</span>
          </div>
        {:else}
          <div class="space-y-4 max-h-[400px] overflow-y-auto pr-1">
            <!-- Enabled toggle -->
            <div class="flex items-center justify-between rounded-lg border border-[var(--border)] bg-[var(--background)] p-3">
              <span class="text-sm font-medium text-[var(--muted-foreground)]">Channel Enabled</span>
              <input
                type="checkbox"
            ...
            {#each activeChannel.configFields as field (field.key)}
              <div class="space-y-1">
                <div class="flex items-center justify-between">
                  <label for="field-{field.key}" class="text-xs font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">
                    {field.label}
                    {#if field.required}
                      <span class="text-[var(--destructive)] font-bold">*</span>
                    {/if}
                  </label>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-[var(--border)]">
          <div class="flex gap-2">
            {#if configSource === "db"}
              <button type="button"
                ...
                class="rounded-lg border border-[var(--destructive)]/30 px-3 py-2 text-xs font-semibold text-[var(--destructive)] hover:bg-[var(--destructive)]/10 transition-colors"
              >
                Delete Config
              </button>
            {/if}
          ...
          <div class="flex gap-2">
            <button type="button"
              ...
              class="rounded-lg border border-[var(--border)] px-4 py-2 text-xs font-semibold text-[var(--muted-foreground)] hover:bg-[var(--accent)] transition-colors disabled:opacity-40"
            >
            ...
            <button type="button"
              ...
              class="rounded-lg bg-[var(--primary)] px-5 py-2 text-xs font-semibold text-white hover:bg-[var(--primary-hover)] transition-colors disabled:opacity-40"
            >
