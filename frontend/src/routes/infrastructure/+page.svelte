...
async function loadCurrent() {
  try {
    loading = true;
    error = null;
    const [cResult, hResult, gResult] = await Promise.all([
      getContainerStats(),
      getHostStats(),
      getGpuStats(),
    ]);
  ...
  try {
    ...
    for (const c of topContainers) {
      ...
      if (cRes.ok) {
        ...
        cMap.set(
          c.name,
          cData.data.map((r: ContainerHistoryRow) => ({
            time: new Date(r.collectedAt),
            value: r.cpuPercent,
          })),
        );
...
$effect(() => {
  ...
  const observer = new ResizeObserver(() => {
    if (cpuCanvas && hostHistory.length > 0) {
      drawTimeSeriesChart(
        cpuCanvas,
        hostHistory.map((h) => h.value),
        { color: "#ef4444", max: 100 },
      );
    }
    if (memCanvas && memHistory.length > 0) {
      drawTimeSeriesChart(
        memCanvas,
        memHistory.map((h) => h.value),
        { color: "#3b82f6", max: 100 },
      );
    }
    if (diskCanvas && diskHistory.length > 0) {
      drawTimeSeriesChart(
        diskCanvas,
        diskHistory.map((h) => h.value),
        { color: "#f59e0b", max: 100 },
      );
    }
  });
...
<div class="space-y-6">
  ...
  {#if error}
    <div class="flex items-center gap-3 rounded-lg border border-[var(--destructive)]/50 bg-[color-mix(in_oklch,var(--destructive)_18%,transparent)] px-4 py-3 text-sm text-[var(--destructive)]">
      ...
      <button type="button" onclick={() => loadCurrent()} class="rounded border border-[var(--destructive)]/50 px-2.5 py-1 text-xs text-[var(--destructive)] hover:bg-[color-mix(in_oklch,var(--destructive)_18%,transparent)] transition-colors">Retry</button>
    </div>
  {/if}
  ...
  {#if loading && !host}
    <div class="space-y-6">
      ...
      <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        <div class="mb-4 h-6 w-40 animate-pulse rounded bg-[var(--accent)]"></div>
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {#each Array(3) as _, i (i)}
            <div class="space-y-2 animate-pulse" style="opacity: {1 - i * 0.15}">
              <div class="h-4 w-16 rounded bg-[var(--accent)]"></div>
              <div class="h-2 w-full rounded-full bg-[var(--accent)]"></div>
            </div>
          {/each}
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        {#each Array(4) as _, i (i)}
          <div class="h-48 animate-pulse rounded-lg border border-[var(--border)] bg-[var(--card)]" style="opacity: {1 - i * 0.15}"></div>
        {/each}
      </div>
    </div>
  {:else}
    ...
    {#if host}
      <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
          ...
          <div>
            <div class="flex items-center justify-between text-sm">
              <span class="text-[var(--muted-foreground)]">Disk</span>
        ...
        {#if host.swap_total_mb !== undefined && host.swap_used_mb !== undefined && host.swap_total_mb > 0}
          <div class="mt-4">
            <div class="flex items-center justify-between text-sm">
              <span class="text-[var(--muted-foreground)]">Swap</span>
        ...
        {#if host.cpu_cores && host.cpu_cores.length > 0}
          <div class="mt-4">
            <p class="mb-2 text-xs text-[var(--muted-foreground)]">Per-core CPU</p>
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {#each host.cpu_cores as core}
                <div class="flex items-center gap-2 text-xs">
                  <span class="w-8 text-[var(--muted-foreground)]">#{core.core}</span>
                  <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--accent)]">
        ...
        {#if host.disks && host.disks.length > 1}
          <div class="mt-4">
            <p class="mb-2 text-xs text-[var(--muted-foreground)]">Disk Partitions</p>
            <div class="space-y-1.5">
              {#each host.disks as disk}
                <div class="flex items-center gap-3 text-xs">
                  <span class="w-20 truncate text-[var(--muted-foreground)]">{disk.mount}</span>
                  <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--accent)]">
    ...
    <!-- GPU resources -->
    {#if gpus.length > 0}
      <div class="rounded-lg border border-[var(--border)] bg-[var(--background)] p-4">
            ...
            <div class="rounded-md border border-[var(--border)] p-3">
              <div class="mb-2 flex items-center justify-between">
                ...
                {#if gpu.temperatureC != null}
                  <span class="text-xs text-[var(--muted-foreground)]">{gpu.temperatureC.toFixed(0)}°C</span>
                {/if}
              </div>
              <div class="mb-1 flex justify-between text-sm">
                <span class="text-[var(--muted-foreground)]">Utilization</span>
                <span class="font-medium">{(gpu.utilizationPercent ?? 0).toFixed(1)}%</span>
              </div>
              <div class="h-2 overflow-hidden rounded-full bg-[var(--accent)]">
              ...
              <div class="mt-3 mb-1 flex justify-between text-sm">
                <span class="text-[var(--muted-foreground)]">VRAM</span>
      ...
      <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div class="grid grid-cols-1 gap-6">
            <TimeSeriesChart data={hostHistory} label="CPU %" color="var(--destructive)" unit="%" height={160} />
            <TimeSeriesChart data={memHistory} label="Memory %" color="var(--info)" unit="%" height={160} />
            <TimeSeriesChart data={diskHistory} label="Disk %" color="var(--warning)" unit="%" height={160} />
          </div>
        </div>
        ...
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div class="grid grid-cols-1 gap-6">
            <div class="flex flex-col">
              <h3 class="mb-2 text-sm font-medium text-[var(--muted-foreground)]">CPU %</h3>
              <div class="relative h-40 rounded-lg border border-[var(--border)] bg-[var(--background)] p-2">
                <canvas bind:this={cpuCanvas} class="h-full w-full"></canvas>
              </div>
            </div>
            <div class="flex flex-col">
              <h3 class="mb-2 text-sm font-medium text-[var(--muted-foreground)]">Memory %</h3>
              <div class="relative h-40 rounded-lg border border-[var(--border)] bg-[var(--background)] p-2">
                <canvas bind:this={memCanvas} class="h-full w-full"></canvas>
              </div>
            </div>
            <div class="flex flex-col">
              <h3 class="mb-2 text-sm font-medium text-[var(--muted-foreground)]">Disk %</h3>
              <div class="relative h-40 rounded-lg border border-[var(--border)] bg-[var(--background)] p-2">
    ...
    <div>
      ...
      {#if containers.length === 0}
        <div class="flex flex-col items-center justify-center py-12">
          <svg class="mb-4 h-12 w-12 text-[var(--muted-foreground)] opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
          <p class="text-sm font-medium text-[var(--muted-foreground)]">No containers detected</p>
          <p class="mt-1 text-xs text-[var(--muted-foreground)]">Mount the Docker socket to enable container monitoring</p>
        </div>
      {:else}
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          {#each containers as container (container.id)}
            <a href="/infrastructure/containers/{container.id}" class="block rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 transition-all hover:border-[var(--primary)] hover:shadow-md cursor-pointer">
              <div class="flex items-center justify-between">
                <h3 class="truncate font-medium">{container.name}</h3>
                <span class="rounded-full px-2 py-0.5 text-xs font-medium {container.status === 'running' ? 'bg-[color-mix(in_oklch,var(--success)_18%,transparent)] text-[var(--success)] border border-[var(--success)]/30' : 'bg-[var(--accent)] text-[var(--muted-foreground)] border border-[var(--border)]'}">
                  {container.status}
                </span>
              </div>
              <p class="mt-1 truncate text-xs text-[var(--muted-foreground)]">{container.image}</p>
              <div class="mt-4 grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p class="text-xs text-[var(--muted-foreground)]">CPU</p>
                  <p class="font-medium">{(container.cpu_percent ?? 0).toFixed(1)}%</p>
                </div>
                <div>
                  <p class="text-xs text-[var(--muted-foreground)]">Memory</p>
                ...
                <div>
                  <p class="text-xs text-[var(--muted-foreground)]">Net RX</p>
                ...
                <div>
                  <p class="text-xs text-[var(--muted-foreground)]">Net TX</p>
              ...
              {#if container.restart_count > 0}
                <div class="mt-3 flex items-center gap-1.5 text-xs text-[var(--warning)]">
              ...
              {/if}
              <p class="mt-3 text-xs text-[var(--primary)]">View details →</p>
            </a>
          {/each}
        </div>
      {/if}
      ...
      <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          {#each [...containerHistory.entries()] as [name, data]}
            <TimeSeriesChart {data} label={name} color="var(--violet)" unit="%" height={140} />
          {/each}
        </div>
      </div>
    {/if}
