...
<div class="p-6 max-w-[1400px] mx-auto space-y-6">
  ...
  <div class="space-y-4">
    <button type="button"
      ...
      class="inline-flex items-center gap-2 text-sm text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors"
    >
    ...
    {#if loading}
      <div class="h-12 w-1/3 animate-pulse rounded bg-[var(--card)]"></div>
    {:else if error}
      <div class="rounded-lg border border-[var(--destructive)]/50 bg-[color-mix(in_oklch,var(--destructive)_18%,transparent)] p-4 text-sm text-[var(--destructive)]">
        {error}
      </div>
    {:else if release}
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <div class="flex flex-wrap items-center gap-3">
            <h1 class="text-3xl font-extrabold tracking-tight text-[var(--foreground)]">
          ...
          <p class="mt-2 text-sm text-[var(--muted-foreground)]">
            Project: <span class="font-semibold text-[var(--muted-foreground)]">{getProjectName(release.projectId)}</span>
          </p>
        </div>
      </div>
      ...
      {#if health}
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <!-- Health Score Card -->
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-2">
            <span class="text-xs font-medium text-[var(--muted-foreground)] uppercase tracking-wider">Health Status</span>
            <div class="flex items-baseline gap-2">
              <span class="text-2xl font-bold capitalize {healthColor(health.healthScore)}">{health.healthScore}</span>
            </div>
            <p class="text-xs text-[var(--muted-foreground)]">Based on new issues rate</p>
          </div>

          <!-- New Issues Card -->
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-2">
            <span class="text-xs font-medium text-[var(--muted-foreground)] uppercase tracking-wider">New Issues</span>
            <div class="flex items-baseline gap-2">
              <span class="text-3xl font-extrabold text-[var(--foreground)]">{health.newIssuesCount}</span>
            </div>
            <p class="text-xs text-[var(--muted-foreground)]">First seen after deployment</p>
          </div>

          <!-- Error Rate Card -->
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-2">
            <span class="text-xs font-medium text-[var(--muted-foreground)] uppercase tracking-wider">Error Rate</span>
            <div class="flex items-baseline gap-2">
              <span class="text-3xl font-extrabold text-[var(--foreground)]">{health.errorRate}</span>
              <span class="text-sm text-[var(--muted-foreground)]">/ hr</span>
            </div>
            <p class="text-xs text-[var(--muted-foreground)]">Events in observation window</p>
          </div>

          <!-- Window Card -->
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-5 space-y-2">
            <span class="text-xs font-medium text-[var(--muted-foreground)] uppercase tracking-wider">Observation Window</span>
            <div class="flex items-baseline gap-2">
              <span class="text-3xl font-extrabold text-[var(--foreground)]">{Math.round(health.windowHours)}</span>
              <span class="text-sm text-[var(--muted-foreground)]">hours</span>
            </div>
            <p class="text-xs text-[var(--muted-foreground)]">Deployed {timeAgo(release.deployedAt ?? release.createdAt)}</p>
          </div>
        </div>
      {/if}
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <h2 class="text-xl font-bold text-[var(--foreground)]">Issues in this Environment</h2>
          <span class="text-xs text-[var(--muted-foreground)]">{totalIssues} issues found</span>
        </div>

        {#if issuesLoading && issues.length === 0}
          <div class="space-y-2">
            {#each Array(4) as _, i (i)}
              <div class="h-14 animate-pulse rounded-lg bg-[var(--card)]" style="opacity: {1 - i * 0.15}"></div>
            {/each}
          </div>
        {:else if issues.length === 0}
          <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--card)] py-16">
            ...
            <p class="text-sm font-medium text-[var(--muted-foreground)]">No issues in this environment</p>
            <p class="mt-1 text-xs text-[var(--muted-foreground)]">Excellent! No errors have been reported for this release's environment.</p>
          </div>
        {:else}
