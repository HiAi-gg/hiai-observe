<script lang="ts">
import { type AlertHistoryEntry, type AlertRule, getAlertHistory, getAlerts } from "$lib/api";
...
function statusDot(entry: AlertHistoryEntry): { color: string; label: string } {
  if (entry.resolvedAt) return { color: "var(--success)", label: "Resolved" };
  return { color: "var(--destructive)", label: "Triggered" };
}
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold text-[var(--foreground)]">Alert History</h1>
    <span class="text-sm text-[var(--muted-foreground)]">{total} events</span>
  </div>
  ...
  <div class="flex flex-wrap items-center gap-3">
    <select
      ...
      class="rounded-md border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-sm focus:border-[var(--primary)] focus:outline-none"
    >
    ...
    <div class="flex items-center gap-1 rounded-md border border-[var(--border)] p-0.5">
      {#each timeRanges as tr (tr.label)}
        <button type="button"
          ...
          class="rounded px-3 py-1.5 text-xs font-medium transition-colors {selectedRange === tr.label ? 'bg-[var(--primary)] text-white' : 'text-[var(--muted-foreground)] hover:bg-[var(--accent)]'}"
        >{tr.label}</button>
      {/each}
    </div>
  </div>
  ...
  {:else if entries.length === 0}
    <div class="flex flex-col items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--card)] py-16">
      <svg class="mb-3 h-10 w-10 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <p class="text-sm font-medium text-[var(--muted-foreground)]">No alert events</p>
      <p class="mt-1 text-xs text-[var(--muted-foreground)]">Alerts will appear here when triggered</p>
    </div>
  {:else}
    <div class="space-y-2">
      {#each entries as entry (entry.id)}
        ...
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)]">
          <button type="button"
            ...
            class="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--accent)]/30"
          >
            ...
            <svg class="h-4 w-4 shrink-0 text-[var(--muted-foreground)] transition-transform {expandedId === entry.id ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          {#if expandedId === entry.id}
            <div class="border-t border-[var(--border)] px-4 py-3 space-y-2">
              <div class="grid grid-cols-2 gap-4 text-xs">
                <div>
                  <span class="text-[var(--muted-foreground)]">Triggered:</span>
                  <span class="ml-1 text-[var(--muted-foreground)]">{formatTimestamp(entry.triggeredAt)}</span>
                </div>
                {#if entry.resolvedAt}
                  <div>
                    <span class="text-[var(--muted-foreground)]">Resolved:</span>
                    <span class="ml-1 text-[var(--muted-foreground)]">{formatTimestamp(entry.resolvedAt)}</span>
                  </div>
                {/if}
              </div>
              {#if entry.context}
                <div>
                  <p class="mb-1 text-xs font-medium text-[var(--muted-foreground)]">Context:</p>
                  <pre class="overflow-x-auto rounded-md bg-[var(--background)] p-3 text-xs text-[var(--muted-foreground)]">{JSON.stringify(entry.context, null, 2)}</pre>
                </div>
              {/if}
            </div>
          {/if}
      ...
      <div class="flex items-center justify-between pt-2">
        ...
        <div class="flex items-center gap-2">
          <button type="button"
            ...
            disabled={!hasPrev}
            class="rounded-md border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--muted-foreground)] transition-colors hover:bg-[var(--accent)] disabled:opacity-40"
          >
            Previous
          </button>
          <button type="button"
