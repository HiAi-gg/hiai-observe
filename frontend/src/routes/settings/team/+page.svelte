...
<div class="space-y-6">
  ...
  <div class="flex items-center gap-3">
    <a href="/settings" class="text-sm text-[var(--primary)] hover:underline">Settings</a>
    <svg class="h-4 w-4 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 5l7 7-7 7"/></svg>
    <h1 class="text-2xl font-bold text-[var(--foreground)]">Team</h1>
  </div>
  ...
  <div class="flex items-center justify-between">
    <p class="text-sm text-[var(--muted-foreground)]">{total} team members</p>
    <button type="button"
  ...
  {#if showCreateForm}
    <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4 space-y-4">
      <h3 class="text-sm font-semibold text-[var(--muted-foreground)]">Add Team Member</h3>
  ...
  {:else if members.length === 0}
    <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--card)] py-20">
      ...
      <p class="text-sm font-medium text-[var(--muted-foreground)]">No team members</p>
      <p class="mt-1 text-xs text-[var(--muted-foreground)]">Add team members to assign issues and collaborate</p>
    </div>
  {:else}
    <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-[var(--border)] bg-[var(--accent)]/50">
          <tr>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Member</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Email</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Role</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Project</th>
        ...
        <tbody class="divide-y divide-[var(--border)]">
          {#each members as member (member.id)}
            {#if editingId === member.id}
              <!-- Edit row -->
              <tr class="bg-[var(--primary)]/5">
                <td class="px-4 py-3">
                  <input type="text" bind:value={editName} class="w-full rounded border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-sm focus:border-[var(--primary)] focus:outline-none" />
                </td>
                <td class="px-4 py-3">
                  <input type="email" bind:value={editEmail} class="w-full rounded border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-sm focus:border-[var(--primary)] focus:outline-none" />
                </td>
                <td class="px-4 py-3">
                  <select bind:value={editRole} class="rounded border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-sm focus:border-[var(--primary)] focus:outline-none">
                    {#each roles as role}
                      <option value={role}>{role}</option>
                    {/each}
                  </select>
                </td>
                <td class="px-4 py-3 text-xs text-[var(--muted-foreground)]">{getProjectName(member.projectId)}</td>
                <td class="px-4 py-3 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <button type="button" onclick={saveEdit} class="rounded bg-[var(--primary)] px-2.5 py-1 text-xs font-medium text-white hover:bg-[var(--primary-hover)]">Save</button>
                    <button type="button" onclick={cancelEdit} class="rounded border border-[var(--border)] px-2.5 py-1 text-xs text-[var(--muted-foreground)] hover:bg-[var(--accent)]">Cancel</button>
                  </div>
                </td>
              </tr>
            {:else}
              ...
              <tr class="transition-colors hover:bg-[var(--primary)]/5">
                <td class="px-4 py-3.5">
                  <div class="flex items-center gap-3">
                    <div class="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--accent)] text-xs font-medium text-[var(--muted-foreground)]">
                      {getInitials(member.name)}
                    </div>
                    <span class="font-medium text-[var(--foreground)]">{member.name}</span>
                  </div>
                </td>
                <td class="px-4 py-3.5 text-[var(--muted-foreground)]">{member.email}</td>
                <td class="px-4 py-3.5">
                  <span class="rounded-full px-2 py-0.5 text-xs font-medium {roleBadgeColor(member.role)}">{member.role}</span>
                </td>
                <td class="px-4 py-3.5 text-xs text-[var(--muted-foreground)]">{getProjectName(member.projectId)}</td>
