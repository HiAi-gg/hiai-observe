...
async function load(silent = false) {
  try {
    ...
    // Guard against a malformed payload returning no `total` (defensive —
    // api.getTraces() already defaults this to 0, but keep the page safe
    // if the helper contract ever changes).
...
function durationColor(ms: number): string {
  if (ms < 1000) return "text-[var(--success)]";
  if (ms < 5000) return "text-[var(--warning)]";
...
<div class="p-6 max-w-[1400px] mx-auto space-y-6">
  ...
  <div class="flex items-end justify-between">
    <div>
      <h1 class="text-2xl font-bold text-[var(--foreground)]">Traces</h1>
      <p class="mt-1 text-sm text-[var(--muted-foreground)]">{total} total traces</p>
    </div>
  </div>
  ...
  <!-- Filters bar -->
  <div class="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
    <div class="relative flex-1 min-w-[180px] max-w-xs">
      ...
      <input
        ...
        class="w-full rounded-lg border border-[var(--border)] bg-[var(--accent)] py-2 pl-9 pr-3 text-sm text-[var(--foreground)] placeholder-[var(--muted-foreground)] transition-colors focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]/30"
      />
    </div>
    <div class="relative flex-1 min-w-[180px] max-w-xs">
      <svg class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
      <input
        type="text"
    ...
    <select
      ...
      class="rounded-lg border border-[var(--border)] bg-[var(--accent)] px-3 py-2 text-sm text-[var(--foreground)] transition-colors focus:border-[var(--primary)] focus:outline-none"
    >
    ...
    {#if workflowFilter || agentFilter || statusFilter}
      <button type="button"
        ...
        class="rounded-lg px-3 py-2 text-xs font-medium text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--accent)] transition-colors"
      >
        Clear filters
      </button>
    {/if}
  ...
  {:else if traces.length === 0}
    <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--card)] py-20">
      ...
      <p class="text-sm font-medium text-[var(--muted-foreground)]">No traces found</p>
    ...
    <div class="overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--card)]">
      <table class="w-full text-left text-sm">
        <thead class="border-b border-[var(--border)] bg-[var(--accent)]/50">
          <tr>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Trace ID</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Workflow</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Agent</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider text-right">Duration</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider text-right">Tokens</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Status</th>
            <th class="px-4 py-3 font-medium text-[var(--muted-foreground)] text-xs uppercase tracking-wider">Time</th>
          </tr>
        </thead>
          ...
          {#each traces as trace (trace.id)}
            ...
            >
              <td class="px-4 py-3.5">
                <code class="rounded bg-[var(--accent)] px-1.5 py-0.5 text-xs font-mono text-[var(--muted-foreground)] group-hover:text-[var(--primary)] transition-colors">{(trace.trace_id || trace.id || "").slice(0, 12)}…</code>
              </td>
              <td class="px-4 py-3.5 font-medium text-[var(--foreground)]">{trace.workflow || trace.name}</td>
              <td class="px-4 py-3.5 text-[var(--muted-foreground)]">{trace.agent || "-"}</td>
              <td class="px-4 py-3.5 text-right font-mono text-xs {durationColor(trace.duration_ms)}">{formatDuration(trace.duration_ms)}</td>
              <td class="px-4 py-3.5 text-right tabular-nums text-[var(--muted-foreground)]">{trace.tokens_used?.toLocaleString() ?? "-"}</td>
              <td class="px-4 py-3.5">
                <StatusBadge status={trace.status} size="sm" />
              </td>
              <td class="px-4 py-3.5 text-xs text-[var(--muted-foreground)]">{timeAgo(trace.start_time)}</td>
            </tr>
          {/each}
        </tbody>
      </table>
