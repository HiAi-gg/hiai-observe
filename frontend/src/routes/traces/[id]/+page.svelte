...
function spanColor(status: string): string {
  ...
  if (status === "ok" || status === "success") return "bg-[var(--success)]";
  return "bg-[var(--accent)]";
}

function spanBarColor(status: string): string {
  if (status === "error") return "bg-[var(--destructive)]/80 hover:bg-[var(--destructive)]";
  if (status === "ok" || status === "success")
    return "bg-[var(--success)]/80 hover:bg-[var(--success)]";
  return "bg-[var(--accent)]/80 hover:bg-[var(--accent)]";
}

const orderedSpans = $derived(trace ? buildSpanTree(trace.spans, trace.start_time) : []);
...
<div class="p-6 max-w-[1400px] mx-auto space-y-6">
  <!-- Back link -->
  <a href="/traces" class="inline-flex items-center gap-1.5 text-sm text-[var(--primary)] hover:underline transition-colors">
  ...
  {#if loading}
    <div class="space-y-4">
      <div class="h-10 w-1/2 animate-pulse rounded-lg bg-[var(--card)]"></div>
      <div class="h-6 w-1/3 animate-pulse rounded-lg bg-[var(--card)]"></div>
  ...
  {:else if trace}
    ...
    <div class="space-y-3">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-[var(--foreground)]">{trace.workflow || trace.name}</h1>
          <code class="mt-1 inline-block rounded bg-[var(--accent)] px-2 py-0.5 font-mono text-xs text-[var(--muted-foreground)]">{trace.trace_id}</code>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3">
          <p class="text-xs text-[var(--muted-foreground)]">Duration</p>
          <p class="mt-1 text-lg font-semibold text-[var(--foreground)]">{formatDuration(totalDuration)}</p>
        </div>
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3">
          <p class="text-xs text-[var(--muted-foreground)]">Spans</p>
          <p class="mt-1 text-lg font-semibold text-[var(--foreground)]">{trace.spans.length}</p>
        </div>
        <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3">
          <p class="text-xs text-[var(--muted-foreground)]">Total Tokens</p>
          <p class="mt-1 text-lg font-semibold text-[var(--foreground)]">{totalTokens > 0 ? totalTokens.toLocaleString() : "-"}</p>
        </div>
        {#if trace.agent}
          <div class="rounded-lg border border-[var(--border)] bg-[var(--card)] p-3">
            <p class="text-xs text-[var(--muted-foreground)]">Agent</p>
            <p class="mt-1 text-lg font-semibold text-[var(--foreground)]">{trace.agent}</p>
          </div>
        {/if}
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6">
      ...
      <div class="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--card)]">
        <div class="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--accent)]/50 px-4 py-2.5">
          <svg class="h-4 w-4 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
          <h2 class="text-sm font-semibold text-[var(--muted-foreground)]">Span Timeline</h2>
        </div>

        {#if orderedSpans.length === 0}
          <div class="flex flex-col items-center justify-center py-16">
            <svg class="mb-3 h-8 w-8 text-[var(--muted-foreground)] opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
            <p class="text-sm text-[var(--muted-foreground)]">No spans recorded</p>
          </div>
        {:else}
          <!-- Time ruler -->
          <div class="flex items-center border-b border-[var(--border)] bg-[var(--accent)]/30 px-4 py-1">
            <div class="w-52 shrink-0"></div>
            <div class="flex flex-1 justify-between text-[10px] text-[var(--muted-foreground)] tabular-nums">
              <span>0ms</span>
          ...
          <div class="divide-y divide-[var(--border)]/50">
            {#each orderedSpans as span (span.span_id)}
              ...
              <button type="button"
                class="flex w-full items-center gap-0 px-4 py-2 text-left transition-colors hover:bg-[var(--primary)]/5 {selectedSpan?.span_id === span.span_id ? 'bg-[var(--primary)]/10' : ''}"
              ...
              >
                ...
                <div class="w-52 shrink-0 truncate pr-3" style="padding-left: {span.depth * 16}px">
                  <div class="flex items-center gap-1.5">
                    ...
                    <span class="truncate text-xs font-medium text-[var(--foreground)]" title={span.name}>{span.name}</span>
                  </div>
                </div>

                <!-- Bar -->
                <div class="relative h-5 flex-1">
                  <div class="absolute inset-0 rounded bg-[var(--accent)]/30"></div>
                  <div
                ...
                <!-- Duration -->
                <span class="w-20 shrink-0 text-right text-xs tabular-nums text-[var(--muted-foreground)]">{formatDuration(span.durationMs)}</span>
              </button>
            {/each}
          </div>
        {/if}
      ...
      <div class="space-y-4">
        ...
        {#if selectedSpan}
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
            <div class="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--accent)]/50 px-4 py-2.5">
              ...
              <h3 class="text-sm font-semibold text-[var(--foreground)] truncate">{selectedSpan.name}</h3>
            </div>
            <div class="p-4 space-y-3">
              <dl class="space-y-2 text-xs">
                <div class="flex justify-between">
                  <dt class="text-[var(--muted-foreground)]">Span ID</dt>
                  <dd class="font-mono text-[var(--muted-foreground)]">{selectedSpan.span_id}</dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-[var(--muted-foreground)]">Parent</dt>
                  <dd class="font-mono text-[var(--muted-foreground)]">{selectedSpan.parent_span_id ?? "root"}</dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-[var(--muted-foreground)]">Kind</dt>
                  <dd class="text-[var(--muted-foreground)]">{selectedSpan.kind}</dd>
                </div>
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                  ...
                  <dd class="text-[var(--muted-foreground)]">{formatDuration(new Date(selectedSpan.end_time).getTime() - new Date(selectedSpan.start_time).getTime())}</dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-[var(--muted-foreground)]">Start</dt>
                  <dd class="text-[var(--muted-foreground)]">{new Date(selectedSpan.start_time).toLocaleTimeString()}</dd>
                </div>
              </dl>

              {#if selectedSpan.attributes && Object.keys(selectedSpan.attributes as Record<string, unknown>).length > 0}
                <div>
                  <h4 class="text-[10px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-2">Attributes</h4>
                  <div class="space-y-1">
                    {#each Object.entries(selectedSpan.attributes as Record<string, unknown>) as [key, value] (key)}
                      <div class="flex items-start gap-2 text-xs">
                        <span class="shrink-0 text-[var(--muted-foreground)]">{key}:</span>
                        <span class="break-all text-[var(--muted-foreground)]">{typeof value === "object" ? JSON.stringify(value) : String(value)}</span>
                      </div>
                    {/each}
                  </div>
                </div>
        ...
        {:else}
          <div class="rounded-xl border border-dashed border-[var(--border)] bg-[var(--card)] p-6 text-center">
            <p class="text-sm text-[var(--muted-foreground)]">Click a span to see details</p>
          </div>
        {/if}
        ...
        {#if tokensByModel.length > 0}
          <div class="rounded-xl border border-[var(--border)] bg-[var(--card)] overflow-hidden">
            <div class="flex items-center gap-2 border-b border-[var(--border)] bg-[var(--accent)]/50 px-4 py-2.5">
              <svg class="h-4 w-4 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
              <h3 class="text-sm font-semibold text-[var(--muted-foreground)]">Tokens by Model</h3>
            </div>
            <div class="p-4 space-y-3">
              {#each tokensByModel as [model, tokens] (model)}
                ...
                <div>
                  <div class="flex items-center justify-between text-xs mb-1">
                    <span class="font-medium text-[var(--foreground)]">{model}</span>
                    <span class="tabular-nums text-[var(--muted-foreground)]">{tokens.toLocaleString()}</span>
                  </div>
                  <div class="h-2 rounded-full bg-[var(--accent)]">
                    <div
                      class="h-full rounded-full bg-[var(--primary)]/70 transition-all"
                      style="width: {barPct}%"
                    ></div>
                  </div>
                </div>
