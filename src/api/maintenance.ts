...
export const maintenanceRoutes = new Elysia({ prefix: "/api/maintenance" })
  ...
  .get("/active/now", async ({ query }) => {
    ...
    const items = await db.select().from(maintenanceWindows)
  ...
  .get("/:id", async ({ params, set }) => {
    const [window] = await db.select().from(maintenanceWindows)
  ...
  .put("/:id", async ({ params, body, set }) => {
    const [existing] = await db.select({ id: maintenanceWindows.id })
