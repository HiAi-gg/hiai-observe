    ...
    async ({ params, query, set }) => {
      ...
      if (query.projectId) {
        const [row] = await db
          ...
          .limit(1);
        dbConfig = row ? decryptConfig(row.config) : null;
      }
      ...
      switch (params.channel) {
        case "telegram": {
          const chatId = dbConfig?.chatId || config.TELEGRAM_CHAT_ID;
          if (!chatId) {
            set.status = 400;
        ...
        case "discord": {
          const webhookUrl = dbConfig?.webhookUrl || config.DISCORD_WEBHOOK_URL;
          if (!webhookUrl) {
            set.status = 400;
            return { error: "No webhook URL configured" };
          }
          result = await sendDiscordAlert(webhookUrl, testPayload, dbConfig ?? undefined);
          break;
        }
        case "email": {
          const to = dbConfig?.to || config.SMTP_USER;
          if (!to) {
            set.status = 400;
            return { error: "No recipient configured" };
          }
          result = await sendEmailAlert({ to, ...testPayload }, dbConfig ?? undefined);
          break;
        }
        case "slack": {
          const webhookUrl = dbConfig?.webhookUrl || config.SLACK_WEBHOOK_URL;
          if (!webhookUrl) {
            set.status = 400;
            return { error: "No webhook URL configured" };
          }
          const { sendSlackAlert } = await import("../alerts/notifiers/slack.js");
          result = await sendSlackAlert(webhookUrl, testPayload, dbConfig ?? undefined);
          break;
        }
        case "webhook": {
          const webhookUrl = dbConfig?.url || config.WEBHOOK_URL;
          if (!webhookUrl) {
            set.status = 400;
            return { error: "No webhook URL configured" };
          }
          const { sendWebhookAlert } = await import("../alerts/notifiers/webhook.js");
          result = await sendWebhookAlert(webhookUrl, testPayload, dbConfig ?? undefined);
          break;
        }
        case "pagerduty": {
          const routingKey = dbConfig?.routingKey || config.PAGERDUTY_ROUTING_KEY;
          if (!routingKey) {
            set.status = 400;
            return { error: "No routing key configured" };
          }
          const { sendPagerdutyAlert } = await import("../alerts/notifiers/pagerduty.js");
          result = await sendPagerdutyAlert(routingKey, testPayload, dbConfig ?? undefined);
          break;
        }
        case "teams": {
          const webhookUrl = dbConfig?.webhookUrl || config.TEAMS_WEBHOOK_URL;
          if (!webhookUrl) {
            set.status = 400;
            return { error: "No webhook URL configured" };
          }
          const { sendTeamsAlert } = await import("../alerts/notifiers/teams.js");
          result = await sendTeamsAlert(webhookUrl, testPayload, dbConfig ?? undefined);
          break;
        }
        case "ntfy": {
          const topic = dbConfig?.topic || config.NTFY_TOPIC;
          if (!topic) {
            set.status = 400;
            return { error: "No topic configured" };
          }
          const { sendNtfyAlert } = await import("../alerts/notifiers/ntfy.js");
          result = await sendNtfyAlert(topic, testPayload, dbConfig ?? undefined);
          break;
        }
        case "gotify": {
          const server = dbConfig?.server || config.GOTIFY_SERVER;
          if (!server) {
            set.status = 400;
            return { error: "No server URL configured" };
          }
          const { sendGotifyAlert } = await import("../alerts/notifiers/gotify.js");
          result = await sendGotifyAlert(server, testPayload, dbConfig ?? undefined);
          break;
        }
        case "pushover": {
          const userKey = dbConfig?.userKey || config.PUSHOVER_USER_KEY;
          if (!userKey) {
            set.status = 400;
            return { error: "No user key configured" };
          }
          const { sendPushoverAlert } = await import("../alerts/notifiers/pushover.js");
          result = await sendPushoverAlert(userKey, testPayload, dbConfig ?? undefined);
          break;
        }
        default:
...
function isConfigured(config: Record<string, string> | null, channel: string): boolean {
  ...
  switch (channel) {
    ...
    case "ntfy":
      return !!config.topic;
    case "gotify":
      return !!(config.server && config.token);
    case "pushover":
...
function getEnvConfig(channel: string): Record<string, string> {
  switch (channel) {
    ...
    case "discord":
      return { webhookUrl: config.DISCORD_WEBHOOK_URL || "" };
    case "email":
      return {
    ...
    case "slack":
      return { webhookUrl: config.SLACK_WEBHOOK_URL || "" };
    case "webhook":
      return {
    ...
    case "pagerduty":
      return { routingKey: config.PAGERDUTY_ROUTING_KEY || "" };
    case "teams":
      return { webhookUrl: config.TEAMS_WEBHOOK_URL || "" };
    case "ntfy":
      return {
    ...
    case "gotify":
      return {
        server: config.GOTIFY_SERVER || "",
        token: config.GOTIFY_TOKEN || "",
      };
    case "pushover":
      return {
        userKey: config.PUSHOVER_USER_KEY || "",
