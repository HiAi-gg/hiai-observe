...
export const releasesRoutes = new Elysia({ prefix: "/api/releases" })
  ...
  // ── Create release ──────────────────────────────────────────────────
  .post("/", async ({ body, set }) => {
    const [created] = await db.insert(releases).values({
  ...
  // ── Release health ──────────────────────────────────────────────────
  .get("/:id/health", async ({ params, set }) => {
    const [release] = await db.select().from(releases)
      .where(eq(releases.id, params.id))
      .limit(1);
    if (!release) {
    ...
    const newIssuesResult = await db.select({ total: count() })
      .from(issues)
      .where(and(
        eq(issues.projectId, release.projectId),
        gte(issues.firstSeen, releaseTime),
      ));

    const newIssuesCount = newIssuesResult[0]?.total ?? 0;

    // Count total events after release
    const eventsResult = await db.select({ total: count() })
      .from(events)
      .where(and(
        eq(events.projectId, release.projectId),
        gte(events.createdAt, releaseTime),
      ));
  ...
  .delete("/:id", async ({ params, set }) => {
    const [existing] = await db.select({ id: releases.id })
      .from(releases)
      .where(eq(releases.id, params.id))
      .limit(1);
    if (!existing) {
