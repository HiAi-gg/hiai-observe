...
export const searchRoutes = new Elysia({ prefix: "/api/search" })
  ...
  .get("/", async ({ query }) => {
    ...
    const matchedIssues = await db.select({
      id: issues.id,
      title: issues.title,
      type: issues.type,
    ...
    const matchedEvents = await db.select({
      id: events.id,
    ...
    const matchedTraces = await db.select({
      id: traces.id,
      name: traces.name,
    ...
    return {
      issues: matchedIssues.map((i) => ({ ...i, projectName: projectMap.get(i.projectId) ?? "Unknown" })),
      events: matchedEvents.map((e) => ({ ...e, projectName: projectMap.get(e.projectId) ?? "Unknown" })),
      traces: matchedTraces.map((t) => ({ ...t, projectName: projectMap.get(t.projectId) ?? "Unknown" })),
    };
  }, {
    query: t.Object({
