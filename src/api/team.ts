...
export const teamRoutes = new Elysia({ prefix: "/api/team" })
  ...
  .post("/", async ({ body, set }) => {
    ...
    const existing = await db.select({ id: teamMembers.id })
      .from(teamMembers)
      .where(and(
        eq(teamMembers.projectId, body.projectId),
        eq(teamMembers.email, body.email),
      ))
      .limit(1);
  ...
  .put("/:id", async ({ params, body, set }) => {
    ...
    if (body.email !== undefined) {
      const duplicate = await db.select({ id: teamMembers.id })
        .from(teamMembers)
        .where(and(
          eq(teamMembers.projectId, existing.projectId),
          eq(teamMembers.email, body.email),
        ))
        .limit(1);
