import { pgTable, text, integer, real, bigint, timestamp, uuid, varchar, jsonb, boolean, index, unique } from "drizzle-orm/pg-core";
...
export const traces = pgTable("traces", {
  ...
  index("traces_total_tokens_idx").on(sql`(${t.attributes}->>'gen_ai.usage.total_tokens')::numeric`),
]);
...
export const savedSearches = pgTable("saved_searches", {
  ...
  index("saved_searches_project_idx").on(t.projectId),
]);
...
export const gpuStats = pgTable("gpu_stats", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  index("gpu_stats_host_time_idx").on(t.hostId, t.collectedAt),
]);
...
export const hostInfo = pgTable("host_info", {
  ...
  index("host_info_host_idx").on(t.hostId),
]);
